...
const INIT_FILTERS = { search: "", hesapAdi: "", sorumlulukMerkezi: "", tarihBaslangic: "", tarihBitis: "" };
...
/* ─── TargetItem (user or project) ─── */
function TargetItem({ item, selected, percentage, onToggle, onPctChange, showPct, singleSelected }) {
    return (
        <div
        ...
        >
            ...
            {showPct && selected && !singleSelected && (
                <div className="mk-target-item-pct" onClick={e => e.stopPropagation()}>
                    <input
                        type="number"
                        min="0"
                        max="100"
            ...
            {showPct && selected && singleSelected && (
                <span style={{ fontSize: 12, color: "var(--green)", fontWeight: 700 }}>%100</span>
            )}
        </div>
    );
}
...
export default function MuhasebeKarlilikSayfasi() {
    ...
    /* Distribution config */
    const [targetType, setTargetType] = useState("project"); // "project" | "user"
    const [selectedMonth, setSelectedMonth] = useState("");
    const [selectedIds, setSelectedIds] = useState([]); // project or user ids
    const [percentages, setPercentages] = useState({}); // id -> number

    /* UI state */
    const [filtersOpen, setFiltersOpen] = useState(false);
    const [filters, setFilters] = useState(INIT_FILTERS);
    const [sortConfig, setSortConfig] = useState({ key: "tarihObj", direction: "desc" });
        ...
        setSelectedIds(prev => {
            ...
            setPercentages(p => {
                ...
                if (exists) {
                    ...
                    // if was single before, reset the old one
                    if (next.length === 2) {
                        const oldId = next[0];
                        np[oldId] = 0;
                    }
    ...
    const handlePctChange = (id, val) => {
        setPercentages(prev => ({ ...prev, [id]: val === "" ? "" : Number(val) }));
    };

    /* ─── Total percentage ─── */
    const totalPct = useMemo(() => {
        if (selectedIds.length <= 1) return 100;
        return round2(selectedIds.reduce((s, id) => s + Number(percentages[id] || 0), 0));
    ...
    const handleFile = async (file) => {
        try {
            setIsLoading(true);
            setError(""); setInfo(""); setRows([]);
            setFileName(file.name);
            const parsed = await parseMuhasebeExcel(file);
            const prepared = (parsed.rows || []).map(r => ({ ...r, selected: false }));
            setRows(prepared);
    ...
    const filteredRows = useMemo(() => {
        ...
        if (filters.search.trim()) {
            ...
            d = d.filter(r => r.rawText?.toLocaleLowerCase("tr-TR").includes(q));
        }
        if (filters.hesapAdi) d = d.filter(r => r.hesapAdi === filters.hesapAdi);
        if (filters.sorumlulukMerkezi) d = d.filter(r => r.sorumlulukMerkeziAdi === filters.sorumlulukMerkezi);
        ...
        d.sort((a, b) => {
            ...
            if (av instanceof Date && bv instanceof Date) return direction === "asc" ? av - bv : bv - av;
    ...
    const selectedRows = useMemo(() => rows.filter(r => r.selected && Number(r.borc || 0) > 0), [rows]);
    const selectedDebt = useMemo(() => round2(selectedRows.reduce((s, r) => s + Number(r.borc || 0), 0)), [selectedRows]);

    const allFilteredSelected = filteredRows.length > 0 && filteredRows.every(r => r.selected);
    ...
    const handleSave = async () => {
        if (!selectedMonth) { showModal("warning", "Ay Seçilmedi", "Lütfen dönem ayı seçin."); return; }
        if (selectedIds.length === 0) { showModal("warning", "Hedef Seçilmedi", "En az 1 kullanıcı veya proje seçin."); return; }
        if (selectedRows.length === 0) { showModal("warning", "Satır Seçilmedi", "Tabloda en az 1 satır seçin."); return; }
        if (selectedIds.length > 1 && totalPct !== 100) {
            showModal("warning", "Yüzde Hatası", `Yüzde toplamı 100 olmalı. Şu an: %${totalPct}`);
            return;
        }

        try {
            setIsSaving(true);
            const selectedTargets = targetList.filter(t => selectedIds.includes(t.id));

            if (targetType === "user") {
                // Insert into kullanici_proje_dagilim or similar table
                const payload = selectedTargets.map((t, i) => {
                    const pct = selectedIds.length === 1 ? 100 : Number(percentages[t.id] || 0);
                    return {
                        kullanici_id: t.id,
                        proje_id: null,
                        dagilim_yuzde: pct,
                        reel_proje_adi: selectedRows.map(r => r.aciklama || r.hesapAdi).join(", ").slice(0, 200),
                    };
                });
                const { error: e } = await supabase.from("kullanici_proje_dagilim").insert(payload);
                if (e) throw e;
            } else {
                const payload = [];

                // 1) Seçili satırları hesap adına göre grupla
                ...
                // 2) Her hesap adı toplamını seçilen projelere oranla dağıt
                // 2) Her hesap adı toplamını seçilen projelere oranla dağıt
                Object.values(groupedByAccount).forEach((group) => {
                    ...
                    selectedTargets.forEach((t, index) => {
                        const pct =
                            selectedIds.length === 1 ? 100 : Number(percentages[t.id] || 0);
                        ...
                        payload.push({
                            ...
                            aciklama: group.aciklamalar.slice(0, 5).join(" | ").slice(0, 250),
                            dagilim_orani: pct,
                        });
                    });
                });
                ...
                if (e) throw e;          }
            showModal("success", "Kaydedildi", `${selectedTargets.length} hedefe toplam ${formatMoney(selectedDebt)} dağıtıldı.`);
        } catch (err) {
            showModal("error", "Hata", err?.message || "Kayıt sırasında hata oluştu.");
        } finally {
            setIsSaving(false);
        }
    };
            ...
            <div className="mk-page">
                ...
                <div className="mk-header">
                    <div className="mk-header-left">
                        ...
                        <h1>Borç Dağıtımı</h1>
                        <p>Excel yükle · Satır seç · Kullanıcı veya projeye dağıt</p>
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <button className="btn btn-ghost" onClick={() => fileInputRef.current?.click()} disabled={isLoading}>
                            📂 Excel Seç
                        </button>
                        <button className="btn btn-ghost" onClick={() => setFiltersOpen(p => !p)}>
                            🔍 {filtersOpen ? "Filtreyi Gizle" : "Filtrele"}
                        </button>
                        <button className="btn btn-primary" onClick={handleSave} disabled={isSaving || !rows.length}>
                ...
                <div
                    ...
                    onDragEnter={e => { e.preventDefault(); e.stopPropagation(); if (!isLoading) setDragActive(true); }}
                    onDragOver={e => { e.preventDefault(); e.stopPropagation(); if (!isLoading) setDragActive(true); }}
                ...
                >
                    {isLoading ? (
                        ...
                        <div className="mk-dropzone-inner">
                            ...
                            <div className="mk-dropzone-title">Excel dosyasını sürükle & bırak</div>
                            <div className="mk-dropzone-sub">veya aşağıdaki butonu kullan</div>
                            <button className="mk-upload-btn" onClick={() => fileInputRef.current?.click()}>
                                📂 Dosya Seç
                            </button>
                        </div>
                    )}
                ...
                <div className="mk-dist-panel">
                    <div className="mk-dist-panel-head">
                        <div>
                            ...
                            <p>Seçili satırların toplam borcunu kullanıcılara veya projelere dağıt</p>
                        </div>
                    ...
                    {/* Target Type Tabs */}
                    <div className="mk-target-tabs">
                        <button
                            className={`mk-target-tab ${targetType === "user" ? "active" : ""}`}
                            onClick={() => handleTargetTypeChange("user")}
                        >
                            <span className="mk-target-tab-icon">👤</span>
                            <span className="mk-target-tab-label">Kullanıcılara Dağıt</span>
                            <span className="mk-target-tab-sub">Tek veya birden fazla kullanıcı</span>
                        </button>
                        <button
                            className={`mk-target-tab ${targetType === "project" ? "active" : ""}`}
                            onClick={() => handleTargetTypeChange("project")}
                        >
                            <span className="mk-target-tab-icon">📁</span>
                            <span className="mk-target-tab-label">Projelere Dağıt</span>
                            <span className="mk-target-tab-sub">Tek veya birden fazla proje</span>
                        </button>
                    </div>
                        ...
                        <div className="mk-target-selector">
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
                                <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text2)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                                    {targetType === "user" ? "Kullanıcılar" : "Projeler"}
                                    {selectedIds.length > 0 && ` · ${selectedIds.length} seçili`}
                                </span>
                                {selectedIds.length > 0 && (
                                    <button className="btn btn-ghost btn-sm" onClick={() => { setSelectedIds([]); setPercentages({}); }}>
                                        Temizle
                                    </button>
                                )}
                            </div>
                            <div className="mk-targets-list">
                                {targetList.map(item => (
                                    <TargetItem
                                        ...
                                        showPct={true}
                                        singleSelected={selectedIds.length === 1 && selectedIds[0] === item.id}
                                    />
                                ))}
                                {targetList.length === 0 && (
                                    <span style={{ fontSize: 12, color: "var(--text3)" }}>
                                        {targetType === "user" ? "Kullanıcı bulunamadı." : "Proje bulunamadı."}
                                    </span>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="mk-dist-footer">
                        <div className="mk-dist-stats">
                            ...
                            <div className="mk-dist-stat">
                                ...
                                <span className="mk-dist-stat-value green">{formatMoney(selectedDebt)}</span>
                            </div>
                            <div className="mk-dist-stat">
                                <span className="mk-dist-stat-label">Hedef</span>
                ...
                {filtersOpen && (
                    <div className="mk-filter-bar">
                        <div className="mk-filter-bar-head">
                            <h3>🔍 Filtreler</h3>
                            <button className="btn btn-ghost btn-sm" onClick={() => setFilters(INIT_FILTERS)}>Temizle</button>
                        </div>
                        <div className="mk-filters-grid">
                            <Field label="Arama">
                                <input type="text" value={filters.search} placeholder="Açıklama, hesap, kod..."
                                    onChange={e => setFilters(p => ({ ...p, search: e.target.value }))} />
                            </Field>
                            <Field label="Hesap Adı">
                                <select value={filters.hesapAdi} onChange={e => setFilters(p => ({ ...p, hesapAdi: e.target.value }))}>
                                    <option value="">Tümü</option>
                                    {selectOptions.hesapAdlari.map(v => <option key={v} value={v}>{v}</option>)}
                                </select>
                            </Field>
                            <Field label="Sorumluluk Merkezi">
                                <select value={filters.sorumlulukMerkezi} onChange={e => setFilters(p => ({ ...p, sorumlulukMerkezi: e.target.value }))}>
                                    <option value="">Tümü</option>
                                    {selectOptions.sorumlulukMerkezleri.map(v => <option key={v} value={v}>{v}</option>)}
                                </select>
                            </Field>
                            <Field label="Başlangıç Tarihi">
                                <input type="date" value={filters.tarihBaslangic}
                                    onChange={e => setFilters(p => ({ ...p, tarihBaslangic: e.target.value }))} />
                            </Field>
                            <Field label="Bitiş Tarihi">
                                <input type="date" value={filters.tarihBitis}
                ...
                <div className="mk-table-panel">
                    <div className="mk-table-head">
                        <div className="mk-table-head-left">
                            ...
                            <p>Filtrelenen: <strong>{filteredRows.length}</strong> · Seçili (borçlu): <strong>{selectedRows.length}</strong></p>
                        </div>
                        <div className="mk-table-head-right">
                            ...
                            <button className="btn btn-ghost btn-sm" onClick={() => setFilteredSelected(false)}>
                                Seçimi temizle
                            </button>
                        </div>
                    </div>
                                ...
                                {filteredRows.length === 0 ? (
                                    ...
                                    <tr key={row.id} className={row.selected ? "mk-row-selected" : ""}>
                                        <td className="mk-sticky mk-check-col">
                                            <input type="checkbox" checked={Boolean(row.selected)}
                                        ...
                                        <td className="mk-aciklama-cell"><div>{row.aciklama || "-"}</div></td>
                                        <td>{row.hesapKodu || "-"}</td>
                                        <td>{row.hesapAdi || "-"}</td>
